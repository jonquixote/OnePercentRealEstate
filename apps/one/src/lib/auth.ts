/**
 * Wave 5 — minimal self-owned session auth.
 *
 * Why not Auth.js: v5 is still beta and its Next 16 compatibility is
 * unverified; this app needs exactly email+password with a signed session
 * cookie, which is ~100 lines with zero beta dependencies. If OAuth or
 * magic links land later, swap the issue/verify pair behind the same
 * getSessionUser() seam.
 *
 * Session = HS256 JWT (jose) in an httpOnly SameSite=Lax secure cookie.
 * AUTH_SECRET env is required in production — the app refuses to issue or
 * accept sessions without it (fail closed, no dev-secret fallback in prod).
 */
...
function secretKey(): Uint8Array | null {
  ...
  return null; // prod without AUTH_SECRET -> auth disabled, fail closed
}

...
function enterBlockCooloff(parentLog, blockError) {
    ...
    parentLog.error({ cooloff_ms: currentCooloffMs, resume_at: new Date(blockedUntil).toISOString() }, 'upstream block detected; crawling paused for cool-off');
    ...
    void pool
        .query(`UPDATE crawler_block_state
          SET blocked_at = now(),
              cooloff_until = $1,
...
function resetBlockCooloff(parentLog) {
    ...
    if (wasBlocked) {
        ...
        void pool
          ...
          WHERE id = 1`)
            .catch((err) => parentLog.warn({ err: err.message }, 'failed to clear block state'));
    }
}
...
async function restoreBlockState(parentLog) {
    try {
        const res = await pool.query(`SELECT cooloff_until, consecutive_blocks FROM crawler_block_state WHERE id = 1`);
        const row = res.rows[0];
        if (!row?.cooloff_until)
            return;
        ...
        if (until > Date.now()) {
            blockedUntil = until;
            parentLog.warn({ resume_at: new Date(until).toISOString(), consecutive_blocks: n }, 'restored active block cool-off from DB; crawling paused until it expires');
        }
    }
    catch (err) {
        parentLog.warn({ err: err.message }, 'failed to restore block state at boot');
    }
}
...
async function runnerLoop(id, parentLog) {
    while (!shuttingDown) {
        ...
        try {
            ...
            parentLog.warn({ err: err.message, runner: id }, 'claim query failed; backing off');
...
async function reapStuckJobs(parentLog) {
    ...
    try {
        ...
        parentLog.warn({ err: err.message }, 'stuck-job reaper failed');
    }
}
async function reaperLoop(parentLog) {

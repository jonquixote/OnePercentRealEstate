...
interface ListingRow {
  readonly id: number;
  readonly primary_photo: string | null;
}
...
async function checkMediaHealth(url: string): Promise<number> {
  try {
...
async function processListing(
  ...
  try {
    await pool.query(
      `UPDATE listings SET media_url_status = $1, media_last_checked = now()
       WHERE id = $2`,
      [status, row.id]
    );

    traceLog.info(
      { listing_id: row.id, status, url: row.primary_photo },
      'media health checked'
    );
  } catch (err) {
    traceLog.error(
      { listing_id: row.id, err: String(err) },
...
async function fetchBatch(): Promise<ListingRow[]> {
  try {
...
async function gracefulShutdown(signal: string): Promise<void> {
  ...
  if (inFlight > 0) {
    log.warn(
      { in_flight: inFlight },
      'timeout waiting for in-flight jobs, force exiting'
    );
  }

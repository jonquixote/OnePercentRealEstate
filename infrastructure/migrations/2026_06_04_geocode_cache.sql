-- Wave 7: Geocoding cache + rate-limit resilience.
--
-- The scraper uses Census Bureau (primary) + Nominatim (fallback) for
-- geocoding. Census has no documented rate limit; Nominatim is 1 req/sec.
-- The cache absorbs repeat lookups so after one pass, nearly zero API
-- traffic is needed. Misses expire after 7d so a transient outage doesn't
...
COMMENT ON TABLE geocode_cache IS
  'Permanent cache of geocoding results (Census Bureau primary, Nominatim fallback). Successful lookups have no expiry; misses expire after 7d.';

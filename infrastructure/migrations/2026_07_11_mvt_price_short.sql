-- Add price_short (compact '$1.2M' label) to the high-zoom branch of
-- listings_mvt so the map's z13+ price-pill symbol layer has text to render.
-- Full function replacement; body otherwise identical to 2026_06_16 version.
CREATE OR REPLACE FUNCTION public.listings_mvt(z integer, x integer, y integer, p_listing_status text DEFAULT 'for_sale'::text, one_pct_only boolean DEFAULT false, p_sale_type text DEFAULT 'standard'::text, p_min_price numeric DEFAULT NULL::numeric, p_max_price numeric DEFAULT NULL::numeric, p_min_beds integer DEFAULT NULL::integer, p_min_baths numeric DEFAULT NULL::numeric, p_property_type text DEFAULT NULL::text)
 RETURNS bytea
 LANGUAGE plpgsql
    ...
    ELSE
        ...
        FROM (
            SELECT
                ...
                l.price_cut_pct,
                -- price_short: pre-formatted pill label for the map's symbol
                -- layer (MapLibre expressions lack compact number notation).
                CASE
                  WHEN l.price >= 10000000 THEN '$' || round(l.price/1000000.0)::text || 'M'
                  WHEN l.price >= 1000000 THEN '$' || round(l.price/1000000.0, 1)::text || 'M'
                  WHEN l.price >= 1000 THEN '$' || round(l.price/1000.0)::text || 'K'
                  WHEN l.price > 0 THEN '$' || l.price::text
                  ELSE ''
                END AS price_short,

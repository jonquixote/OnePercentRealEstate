-- 2026_06_20_mvt_sale_type_param.sql
-- Make the map (pg_tileserv MVT tiles) distress-aware. Adds a p_sale_type
-- parameter (default 'standard' = canonical view, no double-count). The fast
-- pre-aggregated MV branch is gated to the standard view (the MV is standard-only
-- after 2026_06_20_mv_cluster_tiles_standard.sql); distress views fall to the
-- on-the-fly branches which filter sale_type.
--
-- DROP + CREATE (not CREATE OR REPLACE) because the arg count changes. After this,
-- restart pg_tileserv so it re-introspects and forwards the new p_sale_type param.

-- Smart rent estimation function (fixed: flattened inner DECLARE block)
...
-- Haversine distance fallback when PostGIS geography is unavailable
CREATE OR REPLACE FUNCTION haversine_miles(
    lat1 NUMERIC, lon1 NUMERIC,
    lat2 NUMERIC, lon2 NUMERIC
) RETURNS NUMERIC AS $$
DECLARE
    R CONSTANT NUMERIC := 3958.8;
    dlat NUMERIC;
    dlon NUMERIC;
    a NUMERIC;
    c NUMERIC;
...
-- v2: property_type aware (non-rentable land/lots return 0)
-- v3: flattened inner DECLARE block (PL/pgSQL allows only one DECLARE per block)
...
BEGIN
    -- 0. Non-rentable property types (land, vacant, etc.) short-circuit
    ...
    -- 2. Comparable rentals with weighted similarity scoring
    ...
    filtered_comps AS (
        SELECT *,
            (
                ...
                + CASE WHEN bedrooms = p_beds THEN 0.25 ELSE 0.15 END
    ...
    -- 3. National average fallback by bedroom count (2024 US averages)
    v_national_fallback := CASE COALESCE(p_beds, 2)
        ...
        WHEN 4 THEN 2350
        WHEN 5 THEN 2750
        ELSE 2000 + (COALESCE(p_beds, 3) - 3) * 400
    END;

    -- 4. Pick best estimate with fallback chain
...
-- Quick estimate by address lookup
...
GRANT EXECUTE ON FUNCTION calculate_smart_rent TO PUBLIC;
GRANT EXECUTE ON FUNCTION quick_rent_estimate TO PUBLIC;

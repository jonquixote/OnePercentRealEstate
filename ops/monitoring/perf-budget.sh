...
# Both apps. apps/two was unwatched entirely until 2026-07-26 — it responded in
# ~7ms and nothing would have noticed if it stopped.
APPS="${PERF_BUDGET_URLS:-http://127.0.0.1:3001/api/admin/perf http://127.0.0.1:3002/api/admin/perf}"
...
budget_for() {
  case "$1" in
    ...
    two.market-series|two.screens|two.layouts|two.screen-alerts) echo 500 ;;
    two.terminal|two.portfolio)                  echo 1000 ;;
...
[[ -z "$JSON" ]] && { echo "[perf-budget] could not read $APP" >&2; continue; }
...
')"
[[ -z "$ROWS" ]] && { echo "[perf-budget] no routes reported yet by $APP"; continue; }

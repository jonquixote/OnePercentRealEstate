...
- [ ] Acceptance: `SELECT count(*) FROM epa_walkability;` ≥ 200,000. Commit any loader fix.
...
- [ ] Acceptance: a decision recorded either way; no silent dead code left.
...
- [ ] Acceptance: user decision recorded in this file; chosen path implemented or task closed.
...
- [ ] Acceptance: free account hits the gate, subscribed test account passes it.
...
- [ ] Audit: `ls /opt/onepercent/backups`, cron, `pg_dump` anywhere? If nothing: nightly `pg_dump -Fc` of postgres to `/var/backups/oper/` (7 daily + 4 weekly rotation) via systemd timer, THEN off-box copy — free tier choice with user: rclone to a free object-storage tier, or scp to the user's home machine. **A backup that lives only on the same VPS is not a backup.**

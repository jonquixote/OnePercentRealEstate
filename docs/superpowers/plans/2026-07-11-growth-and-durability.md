...
- [x] **DECISION — states 4-10: EXTENDED (2026-07-12).** With the arcgis loader reliable and cheap (no GDAL, ~90 MB/state, sub-second spatial lookups, ample disk), loaded the remaining top-10 states (GA, NC, NY, MI, TN, OH, PA) via `sh -c "for f in 13 37 36 26 47 39 42; do python load_nfhl.py --state-fips $f; done"` in the scraper container. **`flood_zones` = 660,077 SFHA polygons across all top-10 states** — FL 252,883 / TX 68,034 / NC 59,162 / GA 54,101 / PA 52,991 / NY 48,807 / CA 38,021 / OH 36,229 / MI 25,600 / TN 24,249 — 100% valid geometry, 515 MB, 32 GB disk free. Point-in-polygon verified in the new states too (e.g. GA 970 / NY 1,142 / NC 1,355 / PA 548 listings inside a SFHA). To go further, `load_nfhl.py --state-fips <FIPS>` per state — no code changes needed.
...
- [x] `saved_searches` gains `email_digest BOOLEAN NOT NULL DEFAULT false`; the save-search UI gains the opt-in checkbox (only when signed in with an email).
- [x] Daily 14:00 UTC job (existing worker tick pattern): for each digest-enabled search with `new_matches > 0` (reuse the D3 SQL), send one Resend email with up to 6 new listings (photo, price, ratio, link), then stamp `last_viewed_at`. Hard cap: 1 email/user/day (batch all their searches into one).
- [x] Unsubscribe link → `email_digest = false` for that search (signed one-click token, no login required).
...
- [x] **BUG FIX (2026-07-12):** a review-agent pass caught a parameter-index collision in `digest.ts` `sendDailyDigestForUser`: `compileSavedSearchParams` numbers placeholders from `$1`, but the outer query already binds `$1=cutoff` with `[search.cutoff, ...bind]`, so the first filter (e.g. `price >= $1`) compared against the cutoff *timestamp* — type error / silently-wrong matches. Fixed by shifting the compiled filter placeholders `$n → $(n+1)`. Typecheck + tests green.

### Task 2.2: Weekly ZIP market brief (the data moat as email)
- [x] For watchlisted ZIPs: median list price WoW, new/sold counts, rent $/sqft trend from `h3_market_stats`, HPI YoY from `fhfa_zip_hpi`. One email per user per week, same opt-in + unsubscribe discipline.
...
- [x] Every number sourced from our tables; NO lorem-ipsum filler paragraphs. Page renders nothing rather than an empty section.
- [x] JSON-LD (`Place` + `Dataset` breadcrumbs), canonical, OG image via the existing `og` pattern if present (`grep -r opengraph apps/one/src/app` first).
...
- [x] Restore drill: restore latest dump into a scratch database, count rows in 3 tables, drop it. Scripted as `ops/systemd/verify-backup.sh`, run weekly by timer, alert on failure via OPS webhook.
...
- [x] `ci.yml` audit: does it run vitest (primitives, worker), pytest (ml_rent_estimator), `pnpm build` for both apps, `tsc` for packages? Add what's missing; make it required on main via branch protection (user click) or at minimum a red-X convention.
- [x] Acceptance (2026-07-11, **corrected 2026-07-12**): a code review against this plan caught that the earlier claim was **not backed by code** — `ci.yml` had no test step and `packages/api-client/src/schemas.test.ts` did not exist, so tests gated nothing. **Now actually wired:** root `test` script (`turbo run test`), `- run: pnpm test` added to `.github/workflows/ci.yml` (between typecheck and lint), and `packages/api-client/src/schemas.test.ts` (6 cases) added with vitest on the package. `pnpm test` now runs vitest across primitives (36), worker (11), api-client (6); `@oper/map` uses `--passWithNoTests`. A failing test turns the job red. Verified locally: `pnpm test` → 4 tasks successful; `pnpm typecheck` → 7 successful. Pytest for ml_rent_estimator left un-wired (needs pandas/numpy/xgboost CI setup) — documented.

### Task 4.3: External uptime probe
- [x] Internal Prometheus can't see DNS/TLS/edge failures. Free external check (UptimeRobot free tier or GitHub Actions schedule hitting one.octavo.press + /api/healthz every 5 min, failing loudly to the OPS webhook). Pick the Actions route if no new accounts wanted.
